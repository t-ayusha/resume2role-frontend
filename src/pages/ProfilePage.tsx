...
function ProfileEditModal({ isOpen, onClose, initialDetails, initialAvatar, onSave }: ProfileEditModalProps) {
  ...
  const profileImages = [] as Array<{ id: string; src: string; label: string }>
...
function ProfilePage() {
  ...
  const storageKeyTheme = 'Resume2Role-theme'

  const getStoredTheme = () => {
    const stored = window.localStorage.getItem(storageKeyTheme)
